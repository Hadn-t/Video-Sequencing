...
def real_time_detection():
    ...
    model = load_model('action3GRU.h5')  # Load your GRU model
    ...
    with mp_holistic.Holistic(min_detection_confidence=0.5, min_tracking_confidence=0.5) as holistic:
        while True:
            ...
            if len(sequence) == 30:
                # **Ensure correct input shape for GRU**
                input_data = np.expand_dims(sequence, axis=0)  # Shape (1, 30, feature_length)
                
                res = model.predict(input_data)[0]  # Get prediction
